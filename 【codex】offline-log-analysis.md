...
经济摘要包含金币范围、逐段列明起止轮次/长度/日/昼夜的空指令区间及最长长度、寻路搜索/计算/扩展计数、`defensePlanning.unsafeDayWork` 及规划截断轮数。空指令仅认非负整型 `commandCount=0`；缺失、null、布尔/浮点/负数等错误类型分别计数。重复轮排除，缺帧及日/昼夜边界切段。基地摘要包含 HP/等级范围、升级券使用**请求**数与相邻帧可见的等级上升数。这些只是日志可见事实；发出 `use` 指令不等于平台执行成功。
